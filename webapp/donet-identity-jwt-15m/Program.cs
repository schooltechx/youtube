...
builder.Services.AddSqlite<HeroDb>("Data Source=Hero.db");
builder.Services.AddIdentity<HeroUser, IdentityRole>()
              .AddEntityFrameworkStores<HeroDb>()
...
async (UserManager<HeroUser> userMgr, RoleManager<IdentityRole> roleMgr,LoginRequest user) =>
{
    ...
    var u = await userMgr.FindByNameAsync(user.username);
    if (u == null)
        return Results.NotFound("User not found");
    if(! await userMgr.CheckPasswordAsync(u,user.password))
        return Results.Unauthorized();
    ...
    claims.Add(new Claim(ClaimTypes.GivenName, u.GivenName));
    claims.Add(new Claim(ClaimTypes.Surname, u.Surname));
...
app.MapPost("/register",[Authorize(Roles = "admin")]
async (UserManager<HeroUser> userMgr, RoleManager<IdentityRole> roleMgr, RegisterRequest user) =>
{
    ...
    var createUserResult = await userMgr.CreateAsync(newUser, user.password);
    if(!createUserResult.Succeeded)
        return Results.BadRequest(createUserResult.ToString());
    foreach (var role in user.roles){
        if(! await roleMgr.RoleExistsAsync(role)){
            var createRoleResult = await roleMgr.CreateAsync(new IdentityRole(role));
        }
        await userMgr.AddToRoleAsync(newUser, role);
    }
    return Results.Ok();
...
record RegisterRequest(string username, string password,
    string givenName,string surname, int hp, int mp, string[] roles);
class HeroDb : IdentityDbContext<HeroUser, IdentityRole, string>
{
    public HeroDb(DbContextOptions<HeroDb> options) : base(options) { }
    public DbSet<HeroUser> HeroUsers => Set<HeroUser>();
}
class HeroUser : IdentityUser
{
